...
<head>
  ...
  <style>
    ...
    body { margin:0; background:#0f1115; color:#e8ebf2; height:100vh; display:flex; }
    .sidebar { width:280px; background:#121822; border-right:1px solid #243041; display:flex; flex-direction:column; }
    .side-top { padding:14px; border-bottom:1px solid #243041; display:flex; gap:8px; }
    .btn { background:#4f7cff; color:white; border:none; padding:9px 12px; border-radius:8px; cursor:pointer; }
    .btn.secondary { background:#2b3546; }
    .chat-list { flex:1; overflow:auto; padding:10px; display:flex; flex-direction:column; gap:8px; }
    .chat-item { background:#182132; border:1px solid #2a3952; border-radius:10px; padding:10px; cursor:pointer; }
    .chat-item.active { border-color:#4f7cff; }
    .chat-title { font-weight:600; font-size:14px; }
    .chat-model { color:#9cb0cc; font-size:12px; margin-top:2px; }
    .main { flex:1; display:flex; flex-direction:column; }
    .topbar { padding:14px 18px; border-bottom:1px solid #243041; display:flex; justify-content:space-between; align-items:center; }
    .status { color:#a1b2cb; font-size:13px; }
    .messages { flex:1; overflow:auto; padding:18px; display:flex; flex-direction:column; gap:10px; }
    .bubble { max-width:900px; padding:12px 14px; border-radius:12px; line-height:1.45; white-space:pre-wrap; }
    .user { align-self:flex-end; background:#29466f; }
    .assistant { align-self:flex-start; background:#1a2434; }
    .composer { border-top:1px solid #243041; padding:14px; display:flex; gap:10px; }
    textarea { flex:1; background:#0f1520; color:#e8ebf2; border:1px solid #2a3952; border-radius:10px; padding:10px; min-height:58px; }
    .settings { display:flex; gap:8px; align-items:center; }
    select { background:#111926; color:#e8ebf2; border:1px solid #2a3952; border-radius:8px; padding:8px; }
    a { color:#9dc0ff; }
  </style>
</head>
<body>
...
<script>
  ...
  document.getElementById('newChat').onclick = async () => {
    ...
    const resp = await fetch('/api/chats', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({title:'New Chat', model})});
    const data = await resp.json();
    chats.push(data);
    activeChatId = data.id;
    renderChats();
  ...
  modelSelect.onchange = async () => {
    ...
    await fetch(`/api/chats/${activeChatId}/model`, {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({model:modelSelect.value})});
    await loadChat(activeChatId);
    await loadChats();
  };
  ...
  document.getElementById('composer').onsubmit = async (e) => {
    ...
    prompt.value = '';
    await fetch(`/api/chats/${activeChatId}/messages`, {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({message})});
    await loadChat(activeChatId);
  };
